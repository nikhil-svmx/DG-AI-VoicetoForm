...
// src/server/localRunner.js

// Optional: not needed on Vercel, but harmless locally if you keep a .env file.
import 'dotenv/config';
import { performance } from 'node:perf_hooks';
...
// ---- OpenAI call using native fetch ----
export async function callOpenAI({ system, user, temperature = 0 }) {
  console.log("In call");
  const apiKey = process.env.OPENAI_API_KEY;
  ...
  // Using the Responses API as in your code
  ...
  // Your existing extraction for the Responses API shape
